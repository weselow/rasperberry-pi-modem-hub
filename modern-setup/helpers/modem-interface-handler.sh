...
update_3proxy_config() {
    ...
    # Если IP соответствует стандартному — статичная запись в конфиге уже актуальна
    ...
    # IP нестандартный — обновляем конфиг с блокировкой файла
...
# FUNC-1: возвращает 0 если конфиг изменился, 1 если нет — чтобы не перезапускать зря
remove_from_3proxy_config() {
    ...
    (
        ...
        # Если файлы идентичны — записей для этой подсети не было
        if cmp -s "$PROXY_CFG" "$temp_cfg"; then
            rm -f "$temp_cfg"
            exit 2  # специальный код: нет изменений
        fi
...
handle_add() {
    ...
    # Перезапускаем 3proxy только если требуется (нестандартный IP)
...
handle_remove() {
    ...
    # иначе обрываем активные соединения через другие модемы без причины
