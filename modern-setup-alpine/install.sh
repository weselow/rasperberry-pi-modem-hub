#!/bin/bash
#
# install.sh - Главный скрипт установки системы управления модемами для Alpine Linux
...
check_alpine() {
    if [ ! -f "/etc/alpine-release" ]; then
        log_error "Этот скрипт предназначен для Alpine Linux"
        log_error "Для Ubuntu/Debian используйте modern-setup/install.sh"
        exit 1
    fi

    local alpine_version=$(cat /etc/alpine-release)
    log_info "Обнаружен Alpine Linux: $alpine_version"
}

show_welcome() {
    clear
    log_header "УСТАНОВКА СИСТЕМЫ УПРАВЛЕНИЯ МОДЕМАМИ (ALPINE LINUX)"

    echo "Эта установка настроит систему для автоматического управления"
    echo "несколькими USB-модемами через 3proxy с использованием udev + OpenRC."
    echo ""
    echo "Будут выполнены следующие шаги:"
    echo "  1. Настройка системных лимитов"
    echo "  2. Установка и настройка 3proxy (OpenRC)"
...
show_completion() {
    ...
    echo "  4. Проверьте 3proxy: rc-service 3proxy status"
    echo ""
    echo "Полезные команды (Alpine Linux + OpenRC):"
    echo ""
    echo "  # Управление 3proxy"
    echo "  rc-service 3proxy status"
    echo "  rc-service 3proxy restart"
    echo "  rc-service 3proxy stop"
    echo ""
...
main() {
    ...
    run_script "1/4" "01-set-limits.sh"
    run_script "2/4" "02-install-3proxy.sh"
    run_script "3/4" "03-configure-network.sh"
    run_script "4/4" "04-setup-udev-rules.sh"
